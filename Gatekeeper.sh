...
if  [ ${#Copias[@]} -eq 0 ]; then
	...
	# Registar o nome da cópia requisitada sem a diretoria
        ...
        if [ -n "$Copia_syslog_dir" ]; then
                echo "Registos de negação de permissão $Nome_copia_syslog:"
                # Pesquisar nas linhas do syslog por could not open file
		awk -v m1="Could not open file" -v m2="Permission denied" '$0~m1 && $0~m2' "$Copia_syslog_dir"
        else
